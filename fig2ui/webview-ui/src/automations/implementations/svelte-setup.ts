// services/implementations/SvelteSetup.ts
import { SetupOptions } from "../../interfaces/framework-setup";
import { BaseFrameworkSetup } from "../cli";

export class SvelteSetup extends BaseFrameworkSetup {
    ...
    public async setupConfig(options: SetupOptions): Promise<void> {
        ...
        if (options.cssFramework === 'tailwindcss' || options.cssFramework === 'scss') {
            await this.setupPreprocessor(options.cssFramework);
        }
    }
    ...
    protected async setupPreprocessor(cssFramework: string): Promise<void> {
